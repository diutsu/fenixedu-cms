...
public class AdminSiteBuilder {
    ...
    public String update(Model model, @PathVariable("builderSlug") String builderSlug,
                         @RequestParam(required = false)  String canViewGroup,@RequestParam(required = false)  String theme,
                         @RequestParam(required = false)  String folder,  @RequestParam(required = false) Set<String> roles ) {
        CmsSettings.getInstance().ensureCanManageSettings();
